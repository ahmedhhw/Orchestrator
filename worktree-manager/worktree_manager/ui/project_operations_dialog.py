...
class ProjectOperationsDialog(ctk.CTkToplevel):
    def __init__(self, master, vm: WorkspaceProjectsViewModel, repos: dict,
                 on_create=None, on_edit=None, existing_project=None):
        ...
        self.title("Edit Project" if self._editing else "New Workspace Project")
    ...
    def _build(self):
        heading = "Edit Project" if self._editing else "New Workspace Project"
        ctk.CTkLabel(
            self, text=heading,
        ...
        confirm_text = "Save Changes" if self._editing else "Create Project"
    ...
    def _prepopulate(self, project):
        self._name_var.set(project.name)
        for entry in project.entries:
            self._add_entry(entry.worktree_path)
